...
int main(int argc, char *argv[])
{
    ...
    desc.add_options()
        ...
        ("algorithm,a", po::value<int>(&algo), "set algorithm (0, 1 or 2)")
    ...
    Solution sol(ins);
    if (algo == -1) {
        switch (ins.stack_number()) {
        case 1: {
            sol = sol_astar_historycuts_1(ins, info);
            break;
        } case 2: {
            sol = sol_astar_historycuts_2(ins, info);
            break;
        } default: {
            sol = sol_astar_historycuts(ins, info);
            break;
        }
        }
    } else {
